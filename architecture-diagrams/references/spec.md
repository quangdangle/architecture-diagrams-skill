...
| `routeAround` | `wires`, `blocks` (both optional): `[[from, to], ...]`, `[id, ...]` | In a hand-placed tab, every wire whose path runs under a block that is neither of its ends (or only the wires listed, and the wires under the blocks listed) gets a path round the blocks: the corner points, and anchors on the sides it leaves and enters by where the wire has none (a pin or a hand-set anchor stays), keeping clear of the other wires, labels and frame borders. A bus drawn as a long thin bar is not in the way. A wire whose end is on a symbol with pins but tied to none is left as it is. The checks list such wires (as advice) with this operation as their fix; a pattern ends with it for its own wires and the wires under its new blocks. In a tab laid out automatically it does nothing. |
| `arrange` | `style`: `stages` | Lays a pipeline out: every top-level group is a stage, drawn as a column; the stages go left to right in the order the wires run between them (feedback wires left out), the blocks of a stage top to bottom in their own order. Blocks in no group (caches, memories) sit above the stage they feed or below the stage that writes them. Wires to the next stage leave a block on the right and enter the next on the left; a block feeding several blocks runs one trunk just outside its frame, a block fed by several one just outside the next frame. Wires back or over a stage run down into a corridor under the columns and up again, one row each, nested so they do not cross. Any other wire that would pass under a block goes round it, labels move to where they cover no block, title or other label, and notes to a free spot near what they are about. The tab becomes hand-placed and is marked `"arranged": "stages"`. |
...
- **What it does**: select one or more blocks or frames and press ✦ AI next to them, or use the editor's AI tab for the whole tab. The request goes to Claude Code on the same computer with the tab (as JSON), the selection, the frame boxes and the current problems, and with the whole design around it: the project's title, the overview of a detail board (blocks, descriptions, wires), the block whose inside a tab holds (its description, ports and wires in the overview), every note of kind constraint, reason, question or to-do in any tab, the AI's earlier changes (its notes) and this session's earlier requests. So each request builds on the rest of the chip and on the requests before it ("the same for HMAC"), and follows the constraints written anywhere. A tab arranged around its buses or as a pipeline (`arranged`: `bus` or `stages`) is arranged again, the same way, after the AI adds blocks, wires or groups. The answer is a list of operations (section 11) plus a one-sentence summary. The page applies them to a copy and checks the result; when an operation fails or new problems appear, it sends them back to the AI, at most twice. The change is shown first (green added, amber changed, red removed) and applied with **Apply** as one undo step; the AI's summary becomes a `change` note by `AI`. One request runs at a time. Edits made while the AI works, or while its change is shown, are kept: the AI's operations are applied again to the diagram as it is then, and when they no longer fit the page says so and nothing changes.
