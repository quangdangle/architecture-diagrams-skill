...
- **Từ sơ đồ tổng quan sang sơ đồ chi tiết:**
  - Bấm **Dựng bản chi tiết** trên sơ đồ tổng quan, tool mở một tab mới. Mỗi khối thành một khung ở đúng vị trí cũ. Mỗi đường nối thành một dây giữa hai cổng trên viền khung, cổng mang tên theo quy ước RTL (`i_`, `o_`, `io_` ở đầu, `_n` cho tín hiệu tích cực mức thấp).
  - Kỹ sư vẽ phần bên trong từng khung bằng tay, hoặc chọn một hay nhiều khung rồi nhờ AI vẽ.
  - Sơ đồ tổng quan có thêm khối hay đường nối thì bản chi tiết tự thêm khung, cổng và dây. Tool không tự xoá gì; chỗ nào không còn khớp thì hiện trong mục Kiểm tra.
  - Khung nào vẽ dày thì bấm **▸ Bên trong** để tách ra tab riêng. Tab riêng có sẵn các cổng của khung trên viền. Thêm, đổi tên hay xoá cổng ở bên nào thì bên kia đổi theo.
- **Ghi chú dán:**
  - Bấm phím N để dán ghi chú vào khối, khung hoặc dây đang chọn. Ghi chú đi theo khi kéo khối.
  - Mỗi ghi chú có loại (ghi chú, ràng buộc, lý do, thay đổi, câu hỏi, việc cần làm, chú giải), ngày và người viết.
  - Tool gợi ý sẵn ghi chú dựa trên sơ đồ, ví dụ tín hiệu đi qua hai miền clock, reset nhả lệch với clock, chân còn trống, chi tiết bộ nhớ và bus.
  - Nút 🗒 ẩn hoặc hiện mọi ghi chú khi cần xem hình cho gọn.
- **AI ngay trong trang:**
  - Chọn một hay nhiều khối rồi bấm ✦ AI cạnh khối và gõ yêu cầu, ví dụ "thêm FIFO nhận vào khung UART". Tab AI bên trái nhận yêu cầu cho cả sơ đồ.
...
2. **Với agent khác qua MCP** (Codex CLI, Cursor, VS Code, Gemini CLI, Claude Desktop): đăng ký `architecture-diagrams/scripts/mcp_server.py`. Agent sẽ có sẵn 14 công cụ. Tám công cụ để đọc định dạng, kiểm tra, dựng trang, mở trình sửa, xuất file, nhập draw.io, quét code và tra ký hiệu. Bốn công cụ để liệt kê mạch mẫu, chèn mạch mẫu, sửa sơ đồ bằng thao tác nhỏ, và lấy gợi ý kèm cách sửa cùng bước tiếp theo. Hai công cụ còn lại dựng bản chi tiết từ sơ đồ tổng quan và tách phần bên trong một khung ra tab riêng.
...
python3 architecture-diagrams/scripts/assist.py soc.json --board -o soc.json
python3 architecture-diagrams/scripts/assist.py soc.json --inside uart --diagram blocks-detail -o soc.json
```

### AI trong trang

Nút ✦ AI và tab AI gọi Claude Code trên chính máy đang mở trang, qua một cầu nối nhỏ:

```bash
python3 architecture-diagrams/scripts/ai_bridge.py
```

- Cầu nối in ra một mã ghép. Dán mã vào tab AI một lần, hoặc mở trang với đuôi `#ai=8765:<mã>`.
- Cần cài Claude Code và đăng nhập một lần (gói Claude Team, Pro hoặc Max là đủ, không cần khoá API).
- Cầu nối chỉ nghe trên máy này (127.0.0.1) và chỉ nhận trang mở từ file, từ localhost hoặc từ trang GitHub Pages của tool.
- Claude chạy với mọi công cụ bị tắt: nó chỉ trả lời, không đọc hay sửa file, không chạy lệnh.
- Sonnet trả lời trong khoảng nửa phút; Opus dành cho yêu cầu khó.
...
**From overview to detail.** "Detail board" turns an overview into a hand-placed tab where every block is a frame in the same arrangement and every connection a wire between named ports on the frames' borders (`i_`/`o_`/`io_`, snake_case, `_n` for active-low). Engineers draw the inside of each frame by hand or ask the AI; the board follows later changes to the overview and never deletes what was drawn (Checks lists what no longer matches). "▸ Inside" moves a frame's content to its own tab with the frame's ports on its border, kept in step both ways.

**Sticky notes.** Key N sticks a note to the selected block, frame or wire, with a kind (note, constraint, reason, change, question, to-do, legend), a date and an author; it moves with what it is attached to. The editor suggests notes from the diagram (clock-domain crossings, reset release, open pins, memory and bus details) and 🗒 hides them for a clean view.

**AI in the page.** Select blocks and press ✦ AI, or use the AI tab for the whole diagram. The answer comes back as editing operations; the page checks the wiring, lets the AI fix its own problems (twice at most), previews the change in colour and applies it as one undo step with a note. It calls Claude Code on the same computer through `scripts/ai_bridge.py` (127.0.0.1 only, pairing code, every Claude tool off, a Claude seat is enough); without the bridge the AI tab copies the request for any chat AI.
...
2. **Any MCP client** (Codex CLI, Cursor, VS Code, Gemini CLI, Claude Desktop): register `architecture-diagrams/scripts/mcp_server.py` (commands above). Tools: `diagram_guide`, `diagram_validate`, `diagram_build`, `diagram_open_editor`, `diagram_export`, `diagram_import_drawio`, `diagram_scan_code`, `diagram_symbols`, `diagram_patterns`, `diagram_insert_pattern`, `diagram_apply_ops`, `diagram_suggest`, `diagram_build_board`, `diagram_open_inside`. On the command line, `scripts/assist.py` does the same (`--patterns`, `--pattern ID --near BLOCK`, `--ops FILE`, `--suggest`, `--board`, `--inside FRAME`).
